#include "onlinedice.hpp"

void onlinedice::reveal(const uint64_t& id, const checksum256& seed) {
    require_auth(REVEALER);
    net_bet bet = find_or_error(id);
    ...
    remove(bet);
    net_result result{.bet_id = bet.id,
                     .player = bet.player,
...
void onlinedice::transfer(const account_name& from,
...
void onlinedice::receipt(const net_bet& bet) {
    require_auth(_self);
}

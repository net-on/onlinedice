...
class onlinedice : public contract {
   public:
    onlinedice(account_name self)
        : contract(self),
          _bets(_self, _self),
    ...
    // @abi action
    void receipt(const net_bet& bet);
   ...
   private:
    ...
    asset compute_referrer_reward(const net_bet& bet) { return bet.amount / 200; }

    uint64_t next_id() {
        net_global global = _global.get_or_default(
            net_global{.current_id = _bets.available_primary_key()});
        global.current_id += 1;
        _global.set(global, _self);
        return global.current_id;
    }

    string referrer_memo(const net_bet& bet) {
        string memo = "bet id:";
    ...
    string winner_memo(const net_bet& bet) {
        string memo = "bet id:";
        string id = uint64_string(bet.id);
        memo.append(id);
        memo.append(" player: ");
    ...
    net_bet find_or_error(const uint64_t& id) {
    ...
    void assert_hash(const checksum256& seed_hash, const uint64_t& expiration) {
        ...
        // save hash
        _hash.emplace(_self, [&](net_hash& r) {
            r.hash = seed_hash;
    ...
    void save(const net_bet& bet) {
        _bets.emplace(_self, [&](net_bet& r) {
            r.id = bet.id;
            r.player = bet.player;
    ...
    void remove(const net_bet& bet) { _bets.erase(bet); }

    void unlock(const asset& amount) {
        net_fund_pool pool = get_fund_pool();
        pool.locked -= amount;
    ...
    net_fund_pool get_fund_pool() {
        net_fund_pool fund_pool{.locked = asset(0, EOS_SYMBOL)};
...
void apply(uint64_t receiver, uint64_t code, uint64_t action) {
    onlinedice thiscontract(receiver);

    if ((code == N(eosio.token)) && (action == N(transfer))) {
        execute_action(&thiscontract, &onlinedice::transfer);
        return;
    }

    if (code != receiver) return;

    switch (action) { EOSIO_API(onlinedice, (receipt)(reveal)) };
    eosio_exit(0);
}
}
